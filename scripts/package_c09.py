"""Audio-only C09 checkpoint: preserve every C08 visual payload exactly."""
...
root=R/'work/candidate-c09';out=R/'skin versions/RedMist-Squidward-Briar-C09.fantome';assert not out.exists()
audio_report=json.loads((R/'evidence/c09_voice_validation.json').read_text());assert audio_report['status']=='PASS OFFLINE; GAMEPLAY PENDING'
for w in (R/'work/candidate-c08').glob('*.wad.client'):shutil.copytree(w,root/w.name,dirs_exist_ok=True)
for w in (R/'work/c09-voice').glob('*.wad.client'):shutil.copytree(w,root/w.name,dirs_exist_ok=True)
sha=lambda b:hashlib.sha256(b).hexdigest();before=R/'work/candidate-c08'
for wadname in ['Briar.wad.client','Common.wad.client']:
 for p in (before/wadname).rglob('*'):
  if p.is_file():assert (root/p.relative_to(before)).read_bytes()==p.read_bytes(),'C09 must change only voice files'
protected=[]
...
(meta/'info.json').write_text(json.dumps({'Name':'Red Mist Squidward Briar - C09','Author':'etqdo','Version':'0.9.0','Description':'More frequent laughter:12of15 eligible attack slots laugh and3scream; all standard/long movement slots laugh. Native voice cadence retained. C08 visual and SFX payloads preserved. Purchase sound remains unresolved, unchanged pending recording. Offline validated; gameplay pending.','Heart':'','Home':''},indent=2))
...
result={'status':'OFFLINE VALIDATED; C09 GAMEPLAY PENDING','package':str(out.relative_to(R)),'sha256':sha(out.read_bytes()),'bytes':out.stat().st_size,'wad_counts':counts,'zip_crc':'PASS','native_path_hashes':'PASS','all_final_payloads_match_staging':'PASS','strict_SKN_BIN':'PASS','protected_C08_visuals_unchanged':protected,'manifests':manifests,'runtime_evidence':'C08 purchase still plays normal sound; manager overlay confirmed correctly applied. C09 changes voice frequency only. Purchase remains unresolved pending recording.'}
(R/'evidence/c09_package_validation.json').write_text(json.dumps(result,indent=2));print(json.dumps({k:v for k,v in result.items() if k not in ['manifests','protected_C08_visuals_unchanged']},indent=2))
